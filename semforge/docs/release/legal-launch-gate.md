...
3. `processors`, `thirdPartyDisclosures`, `overseasTransfers`는 각각 별도의 법적 판단 대상이다.
   ...
   `[]`를 쓰면 안 된다.
4. 통신판매업 신고번호가 실제로 있고 공개 대상이면 입력한다. `null`은 전문가가 적용 여부를
   검토해 공개 번호가 없다고 판단한 경우에만 사용한다.
5. 개인정보 처리방침과 약관의 모든 문안을 실제 운영과 대조한다.
6. 승인 전에는 `release.status`를 `draft`로 유지한다.
7. 승인자가 최종본을 확인한 뒤에만 `status`를 `approved`, `attestation`을
   `paid-beta-legal-review-approved`로 바꾸고 승인자·시각·문서 버전을 기록한다.
8. JSON을 한 줄로 직렬화해 배포 환경의 `LEGAL_RELEASE_MANIFEST`에 넣는다.
