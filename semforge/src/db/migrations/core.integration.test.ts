...
test("web, dispatcher, scheduler, worker, billing, privacy role은 BYPASSRLS가 아니다", async () => {
  const roles = await pg.query<{ rolname: string; rolbypassrls: boolean }>(
    "select rolname, rolbypassrls from pg_roles where rolname in ('semforge_billing', 'semforge_dispatcher', 'semforge_privacy', 'semforge_scheduler', 'semforge_web', 'semforge_worker') order by rolname",
  );
