 ...
 *
 * 주의: 이 데이터는 가상 도메인(*.example)의 가짜 SERP/클릭스트림/링크 그래프다.
 * 데이터 원칙(2026-07-29)에 따라 기본 비활성화 — SEED_DEMO_DATA=1 이 설정된
 * 경우에만 삽입한다 (npm run db:seed:analytics 가 이 플래그를 켜고 실행한다).
 */
export async function seedAnalyticsData(now = Date.now()) {
  if (process.env.SEED_DEMO_DATA !== "1") {
    console.log("[seed] 데모 분석 데이터 생략 — SEED_DEMO_DATA=1 일 때만 삽입됩니다.");
    return;
  }
  console.log("[seed] SERP · 클릭스트림 · 링크 그래프 데모 원천 데이터 (SEED_DEMO_DATA=1)");
