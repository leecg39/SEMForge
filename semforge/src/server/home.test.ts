...
before(async () => {
  ...
  sqlite.exec("INSERT INTO folders (id,workspace_id,name,domain) VALUES ('f1','w1','기본','www.example.com'),('f2','w1','미수집','empty.example.com')");
  sqlite.exec("INSERT INTO ai_visibility_queries (id,workspace_id,domain,query,normalized_query) VALUES ('q1','w1','example.com','alpha','alpha'),('q2','w1','example.com','beta','beta'),('q3','w2','example.com','outside','outside')");
  const snapshot = sqlite.prepare("INSERT INTO ai_visibility_snapshots (id,query_id,aio_present,cited,captured_at) VALUES (?,?,?,?,?)");
  ...
  sqlite.close();
  ({ getFolderMetricStrips, calculateAiVisibility } = await import("@/server/home"));
});
