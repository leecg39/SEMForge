...
/** TalorData 실시간 SERP를 현재 SEO 프로젝트의 국가·기기 조건으로 조회한다. */
export default async function KeywordOverviewPage({
  searchParams,
}: {
  searchParams: Promise<{ project?: string; domain?: string; keyword?: string }>;
}) {
  ...
  return (
    <AppShell
      activeToolkit="seo"
      activeHref="/analytics/keywordoverview/"
      projectContext={{
        label: project.name,
    ...
    >
      <KeywordOverviewDashboard
        initialKeyword={query.keyword?.trim() ?? ""}
        initialTargetDomain={domain}
