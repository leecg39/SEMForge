 ...
 * 축적된 serp_snapshots(TalorData 수집)를 도메인 관점으로 조회한
 ...
 * 검색량·KD·CPC·트래픽처럼 연결된 소스가 없는 지표는 미제공으로 표시한다.
 */
...
const COPY = {
  en: {
    ...
    description:
      "Every keyword this domain ranks for in the accumulated SERP snapshots — collected positions only. Metrics without a connected data source are marked as unavailable. No extra API credits are spent on this screen.",
    domain: "Domain",
  ...
  ko: {
    ...
    title: "오가닉 리서치",
    description:
      "축적된 SERP 스냅샷에서 이 도메인이 랭킹된 모든 키워드를 확인하세요 — 수집된 순위만 표시합니다. 연결된 데이터 소스가 없는 지표는 미제공으로 표시합니다. 이 화면은 추가 API 크레딧을 쓰지 않습니다.",
    domain: "도메인",
...
export function OrganicResearchDashboard({
  ...
  const filteredKeywords = useMemo(() => {
    ...
    if (!needle) return report.topKeywords;
    return report.topKeywords.filter((row) => row.keyword.toLowerCase().includes(needle));
  }, [report, filter]);
    ...
    <div className="mx-auto w-full max-w-[1560px] p-4 sm:p-6">
      ...
      {report && (
        <div className={cn("transition-opacity", status === "loading" && "pointer-events-none opacity-60")}>
          {/* KPI 행: 수집 기반 지표만 live 유지, 소스 없는 지표는 미제공 */}
          <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <MetricUnavailable label={copy.organicTraffic} note={copy.unavailableVolume} />
            <MetricUnavailable label={copy.brandedShare} note={copy.unavailableVolume} />
            <Card action={<LivePill label={copy.liveTag} />}>
              ...
              {report.positionDistribution.map((row) => (
                <li key={row.bucket} className="flex items-center gap-2 text-[12px]">
                  ...
                  <span className="tabular-nums text-a2-text-muted">
                    {preciseFormatter.format(row.keywords)}
                  </span>
                </li>
              ))}
            </ul>
                  ...
                  <table className="w-full min-w-[560px] border-collapse">
                    ...
                    <thead>
                      <tr className="bg-[#f9fafb]">
                        <th
                          scope="col"
                          className="border-b border-app-border px-3 py-2.5 text-left text-[11px] font-semibold uppercase tracking-[0.35px] text-a2-text-muted"
                        >
                          {copy.keyword}
                        </th>
                        <th
                          scope="col"
                          className="border-b border-app-border px-3 py-2.5 text-right text-[11px] font-semibold uppercase tracking-[0.35px] text-a2-text-muted"
                        >
                          {copy.position}
                        </th>
                      </tr>
