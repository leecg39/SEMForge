...
interface AiSearchSummary {
  ...
  checkedAt: string;
  reason?: string;
  bots: { token: string; label: string; allowed: boolean }[];
}
...
const COPY = {
  ko: {
    ...
    themeMarkup: "마크업",
    themeAiSearch: "AI 검색 상태",
    notCollected: "수집 전",
    aiBotsBlocked: (count: number) => `차단된 AI 봇 ${count}개`,
    aiBotsTitle: "robots.txt 의 AI 크롤러 규칙",
    aiBotAllowed: "허용",
    aiBotBlocked: "차단",
    aiRobotsFailed: "robots.txt 를 가져오지 못해 판정할 수 없습니다.",
    aiRobotsCheckedAt: (date: string) => `확인 시각: ${date}`,
    noOverviewYet:
  ...
  en: {
    ...
    themeMarkup: "Markup",
    themeAiSearch: "AI Search Visibility",
    notCollected: "Not collected",
    aiBotsBlocked: (count: number) => `${count} AI bots blocked`,
    aiBotsTitle: "AI crawler rules in robots.txt",
    aiBotAllowed: "Allowed",
    aiBotBlocked: "Blocked",
    aiRobotsFailed: "robots.txt could not be fetched, so AI bot access cannot be determined.",
    aiRobotsCheckedAt: (date: string) => `Checked: ${date}`,
    noOverviewYet:
...
/** 개요 그리드 카드 순서 — Site Health 게이지와 함께 Semrush 테마 보고서 9종을 구성한다 */
...
/** 개요 탭 테마 카드 — 점수(%) 표시. 미측정 테마는 0 이 아니라 "수집 전" 상태를 보여준다 */
function ThemeCard({
  theme,
  copy,
  onOpen,
  ...
  return (
    ...
    >
      ...
      {theme.measurable ? (
        ...
        <span className="mt-2 text-[22px] font-semibold leading-[28px] text-app-text-secondary">
          {copy.notCollected}
        </span>
      )}
      <span className="mt-2 flex items-center justify-between border-t border-app-border pt-2.5 text-[12px]">
        <span className={theme.measurable && theme.affectedPages > 0 ? "font-medium text-app-text" : "text-app-text-secondary"}>
          {!theme.measurable
            ? "–"
            : theme.key === "aiSearch"
              ? copy.aiBotsBlocked(theme.affectedPages)
              ...
              <div className="min-w-0">
                ...
                {auditTab === "themes" && (
                  <div className="flex min-w-0 flex-col gap-4">
                      ...
                      {orderedThemes.map((theme) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {theme.key === "aiSearch" && overview?.aiSearch && (
                              <div className="mt-2.5">
                                <p className="text-[12px] font-medium text-app-text">
                                  {copy.aiBotsTitle}
                                </p>
                                {!overview.aiSearch.ok ? (
                                  <p className="mt-1 text-[12px] text-app-text-secondary">
                                    {copy.aiRobotsFailed}
                                    {overview.aiSearch.reason
                                      ? ` (${overview.aiSearch.reason})`
                                      : ""}
                                  </p>
                                ) : (
                                  <>
                                    <button
                                      ...
                                      className="mt-1.5 flex items-center gap-1 text-[12px] font-medium text-app-blue hover:underline"
                                    >
                                      <span
                                        className={cn(
                                    ...
                                    {expanded && (
                                      <ul className="mt-2 max-h-[220px] space-y-1.5 overflow-y-auto border-t border-app-border pt-2">
                                        {overview.aiSearch.bots.map((bot) => (
                                          <li
                                            key={bot.token}
                                            ...
                                            <span
                                              className={cn(
                                                ...
                                                bot.allowed
                                                  ...
                                                  : "bg-[#fdecef] text-[#a4002a]"
                                              )}
                                            >
                                              {bot.allowed ? copy.aiBotAllowed : copy.aiBotBlocked}
                                            </span>
                                          </li>
                                        ))}
                                      </ul>
                                ...
                                <p
                                  className="mt-2 text-[11px] text-app-text-secondary"
                                  suppressHydrationWarning
                                >
                                  {copy.aiRobotsCheckedAt(
                                    dateFormatter.format(new Date(overview.aiSearch.checkedAt))
                                  )}
                                </p>
                              </div>
                            )}
                    ...
                    <CoreWebVitalsPanel domain={campaign.domain} />
                  </div>
                )}
              </div>
            </section>
