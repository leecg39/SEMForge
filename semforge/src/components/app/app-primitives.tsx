...
/**
 * 소스 없는 지표의 정직한 "미제공" 상태.
 * 연결된 데이터 소스가 없는 지표 카드를 가짜 숫자 대신 이 상태로 표시한다.
 * note 에 향후 소스 안내 한 줄을 넘긴다 (예: "백링크 크롤러 연동 후 제공 예정").
 */
export function MetricUnavailable({
  ...
  const message =
    locale === "ko"
      ? "이 지표는 연결된 데이터 소스가 없습니다"
      : "This metric has no connected data source.";
  return (
    <div
      className={cn(
        "rounded-[8px] border border-dashed border-app-border bg-app-bg p-4",
        className
      )}
    >
      <div className="text-[12px] leading-[16px] text-app-text-secondary">{label}</div>
      <div className="mt-1 text-[24px] font-semibold leading-[32px] text-app-text-secondary/60">—</div>
      <p className="mt-1 text-[12px] leading-[16px] text-app-text-secondary">{message}</p>
      {note && <p className="mt-0.5 text-[11px] leading-[15px] text-app-text-secondary/80">{note}</p>}
    </div>
  );
}
