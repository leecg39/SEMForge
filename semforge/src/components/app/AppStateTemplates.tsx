...
export interface EmptyStateProps {
  ...
  body: string;
  cta?: string;
  /** 설정하면 CTA 가 실제 링크로 렌더된다 (데이터 소스 연결 안내 등). */
  ctaHref?: string;
}

export function EmptyState({ title, body, cta, ctaHref }: EmptyStateProps) {
  const localized = useLocalizedValue({ title, body, cta });
  const ctaClass =
    "mt-[20px] inline-flex h-[36px] items-center rounded-[8px] bg-app-blue px-[20px] text-[13px] font-semibold text-white transition-colors hover:bg-app-blue-dark";
  return (
