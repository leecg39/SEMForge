 ...
 * 패널 트래픽 원천(클릭스트림)의 라이브 소스가 없어 현재는 미제공 상태를 표시한다.
    ...
    <WidgetCard big ariaLabel="Traffic Analytics" className="xl:col-span-4">
      ...
      {tab === "google" ? (
        ...
        <>
          ...
          {!report && (
            <p className={cn("mt-4 rounded-[6px] bg-app-bg px-3 py-2 text-[12px] leading-[18px]", SM.caption)}>
              {ko
                ? "전체 방문 추정(패널 트래픽)은 무료 데이터 소스가 없어 제공하지 않습니다. 자사 사이트의 검색 유입은 Traffic 개요(Search Console)에서 확인할 수 있습니다."
                : "Total traffic estimates (panel) are unavailable — no free data source. Own-site search traffic is available in Traffic Overview (Search Console)."}
            </p>
          )}
        </>
      )}
