...
function buildRows(domain: string, suggestions: Suggestion[]): OrganicResearchSetupKeyword[] {
  const sourceRows = [
    ...suggestions,
    ...FALLBACK_KEYWORDS.map((keyword) => ({ keyword, url: `https://${domain}/` })),
  ]
    .filter(
      (item, index, items) =>
...
export function OrganicResearchSetupDialog({
  ...
  const selectSource = (nextSource: KeywordSource) => {
    ...
    if (nextSource === "gsc") {
      setRows(initialRows.slice(0, 5).map((row) => ({ ...row, source: "gsc" })));
    }
  };
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-10 pb-5 pt-9">
                ...
                <section>
                  ...
                  {(source === "gsc" || source === "organic") && (
                    <p className="mt-4 rounded-[6px] bg-[#f2f8fd] px-4 py-3 text-[13px] text-[#3e4e5c]">
                      {source === "gsc"
                        ? "연결 가능한 Search Console 데이터에서 성과가 높은 키워드를 불러왔습니다."
              ...
              {step === 2 && (
                <section>
                  ...
                  >
                    <option value="desktop">SEMrushBot-Desktop</option>
                    <option value="mobile">SEMrushBot-Mobile</option>
                  </select>
                  <p className="mt-3 text-[13px] text-[#3f434b]">
                    {userAgent === "desktop"
                      ? "Mozilla/5.0 (compatible; SemrushBot-SI/0.97; +http://www.semrush.com/bot.html)"
                      : "Mozilla/5.0 (Linux; Android 13; Mobile; compatible; SemrushBot-SI/0.97)"}
                  </p>
                </section>
              )}
